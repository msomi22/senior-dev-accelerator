...
const problem = defineLearningProblem({
  ...
  intuition: 'dp[a] means the fewest coins needed to make amount a. For example, dp[4] asks how to make amount 4: four 1-coins work, but two 2-coins are better, so dp[4] = 2. We solve every smaller amount this way, then reuse those saved answers to solve 11.',
  mentalPicture: 'A row of boxes from amount 0 to the target amount. Each box is a mini target. The number inside the box is the fewest coins found for that amount.',
  ...
  stepByStepBreakdown: ['Create one dp cell for every amount from 0 through 11.', 'Set dp[0] = 0 because amount 0 needs zero coins.', 'For each amount, try every coin that can fit.', 'Compare the candidate coin counts, such as four 1-coins versus two 2-coins for amount 4.', 'Keep the smallest candidate in dp[amount].', 'Return dp[11] because 11 is the final target.'],
  bruteForceThought: 'Brute force tries combinations repeatedly, such as many different ways to make amount 4, amount 6, or amount 10. DP saves the best answer for each amount once.',
  optimizationJourney: 'DP saves each smaller target amount once, then larger amounts reuse those saved results instead of recomputing combinations.',
  ...
  explanation: 'For every amount, try each coin as the last coin used. If the coin fits, look up the best answer for the remaining amount. For example, when solving amount 4, using coin 1 gives dp[3] + 1 = 3 coins, while using coin 2 gives dp[2] + 1 = 2 coins. We keep 2 because it is fewer.',
