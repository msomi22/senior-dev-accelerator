...
const problem = defineLearningProblem({
  ...
  starterThought: 'Start with one output slot. The answer is everything before that index multiplied by everything after that index.',
  intuition: 'The code names those two sides leftProduct and rightProduct. leftProduct means “product before this index.” rightProduct means “product after this index.” For nums = [1, 2, 3, 4], output[2] skips 3: leftProduct is 1 × 2 = 2, rightProduct is 4, so output[2] = 2 × 4 = 8.',
  mentalPicture: 'Picture the current value in the middle. The left side becomes leftProduct. The right side becomes rightProduct. Multiplying leftProduct × rightProduct gives everything except the middle value.',
  patternSignal: 'When each answer needs values on both sides of one skipped index, look for leftProduct and rightProduct thinking.',
  invariant: 'For every index i: answer[i] = product before i (leftProduct) × product after i (rightProduct). nums[i] is not included in either side when that answer is written.',
  ...
  optimizationJourney: 'The visual multiplies “everything except this value” directly. The optimized code does the same work in two reusable pieces: the first pass writes leftProduct for every index, then the second pass multiplies in rightProduct for every index.',
  stepByStepBreakdown: ['For one slot, “except self” means skip nums[i] and multiply every other value.', 'Split “every other value” into leftProduct and rightProduct.', 'leftProduct is the product of values before index i.', 'rightProduct is the product of values after index i.', 'leftProduct × rightProduct becomes answer[i].'],
  finalPattern: 'Precompute leftProduct and rightProduct around each position.',
