  ...
  questions: [
    {
      ...
      requirements: {
        functional: ['Create a short URL from a long URL', 'Redirect users from short URL to long URL', 'Support optional custom aliases', 'Support link expiry', 'Track basic analytics such as click count'],
        nonFunctional: ['Low redirect latency', 'High availability', 'Read-heavy traffic', 'Safe handling of duplicate or invalid links', 'Observable and debuggable production behavior']
      },
      constraints: ['Assume the system is read-heavy.', 'Assume short links may become hot.', 'Assume users may retry create requests.', 'Assume analytics can be eventually consistent.'],
      hints: ['Start with APIs and the core short-code mapping.', 'Explain why your code generation strategy is safe.', 'Separate redirect latency from asynchronous analytics.', 'Discuss what happens when cache, database, or a region fails.'],
      expectedAnswerOutline: ['Clarify requirements and assumptions', 'Define APIs', 'Design short-code generation', 'Design storage model', 'Explain create and redirect flows', 'Add caching for hot links', 'Handle collisions and uniqueness', 'Discuss scaling and partitioning', 'Discuss reliability and failure modes', 'Discuss security and abuse prevention', 'Add observability and metrics'],
      scoringDictionary: {
        urlShortenerBasics: {
        ...
        urlMapping: {
          terms: ['short code to long url', 'short url to long url', 'hashcode long url', 'lookup original url', 'save long url', 'save short url'],
          synonyms: ['short code mapping', 'mapping from code to original url'],
          proximity: {
            windowSize: 12,
            groups: [['short code', 'short url', 'hashcode'], ['long url', 'original url'], ['mapping', 'lookup', 'table', 'save', 'store']]
          }
        },
        redirect: {
          terms: ['accessing short url', 'user provides short url', 'redirect user', 'redirect to long url', '301', '302', 'return long url'],
          stems: ['redirect']
        },
        redirectDbFallback: {
          terms: ['database lookup on cache miss', 'check database on cache miss', 'cache miss then database', 'if not in cache check database', 'lookup database by short code', 'database miss then 404'],
          synonyms: ['read through cache fallback', 'cache miss database fallback'],
          negativePhrases: ['do not check database', 'do not check the database', 'never check database', 'never check the database', 'do not lookup database', 'do not lookup the database', 'never lookup database', 'skip database', 'skip the database', 'no database lookup', 'database lookup is not needed', 'database is not needed', 'return 404 immediately', 'return 404 on cache miss', 'cache miss return 404', 'cache miss returns 404'],
          proximity: {
            windowSize: 12,
            groups: [
        ...
        customAlias: {
          ...
          proximity: {
            windowSize: 10,
            groups: [['custom alias', 'alias', 'reserved'], ['owner', 'ownership', 'namespace', 'authenticated']]
          }
        },
        urlAbuse: {
        ...
        clickAnalytics: {
          ...
          proximity: {
            windowSize: 12,
            groups: [['analytics', 'click', 'event'], ['queue', 'worker', 'kafka', 'event stream'], ['asynchronous', 'eventual', 'background']]
          }
        }
      },
      scoringRubric: [
        { id: 'requirements', title: 'Requirements Clarification', weight: 10, criteria: [
        ...
        { id: 'short-code-generation', title: 'Short Code Generation', weight: 15, criteria: [
        ...
        { id: 'storage-design', title: 'Storage Design', weight: 10, criteria: [
        ...
        { id: 'read-write-flows', title: 'Read and Write Flows', weight: 10, criteria: [
        ...
        { id: 'scaling-performance', title: 'Scaling and Performance', weight: 15, criteria: [
        ...
        { id: 'reliability-consistency', title: 'Reliability and Consistency', weight: 15, criteria: [
          { id: 'high-availability', label: 'High availability', points: 4, concepts: ['availability', 'loadBalancing', 'orchestration', 'replication'], aliases: ['high availability'] },
        ...
        { id: 'security-abuse', title: 'Security and Abuse Prevention', weight: 10, criteria: [
        ...
        { id: 'observability', title: 'Observability', weight: 10, criteria: [
      ...
      commonWeakAnswers: ['Only says use Base62 without explaining why.', 'Mentions Redis/PostgreSQL/Kafka without explaining trade-offs.', 'Ignores collision handling.', 'Ignores redirect latency.', 'Ignores abuse prevention.', 'Ignores failure modes and observability.'],
